...
def val(val_loader,net,criterion,device):
    ...
    log = OrderedDict([('val_loss', val_loss.avg), 
                       ('F1 Score', evaluater.f1_score()),
                       ('Sensitivity', evaluater.confusion_matrix()[3]),
                       ('Specifity', evaluater.confusion_matrix()[2]),
                       ('Accuracy', evaluater.confusion_matrix()[1]),
                       ('AUC-ROC', evaluater.auc_roc()),
                       ('Mean-IOU',evaluater.jaccard_index()),
                       ])
    return log