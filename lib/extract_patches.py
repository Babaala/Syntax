...
def see_load_file_path_txt(file_path):
    img_list = []
    with open(file_path, 'r') as file_to_read:
        while True:
            lines = file_to_read.readline().strip()  # read a line
...
def see_load_data(data_path_list_file):
    print('\033[0;33mload data from {} \033[0m'.format(data_path_list_file))
    img_list = see_load_file_path_txt(data_path_list_file)
    imgs = None
    for i in range(len(img_list)):
        img = np.asarray(readImg(img_list[i]))
        if len(img.shape) < 3:
...
def see_get_data_test_overlap(test_data_path_list, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original = see_load_data(test_data_path_list)
    test_imgs = my_PreProc(test_imgs_original)
...
def post_process(binary_img):
    ...
    size, _, h, w = binary_img.shape
    out = np.zeros((size, 1, h, w), dtype=np.uint8)
    for idx in range(size):
        img_color = np.zeros((1, h, w), dtype=np.uint8)
        current_img = binary_img[idx,:,:,:].squeeze(0)
        retval, labels, stats, centroids = cv2.connectedComponentsWithStats(current_img)
...
def pred_only_in_FOV(data_imgs,data_masks,FOVs,largest_connected=False):
    assert (len(data_imgs.shape)==4 and len(data_masks.shape)==4)  #4D arrays
    height = data_imgs.shape[2]
    width = data_imgs.shape[3]
    if largest_connected:
        binary = data_imgs >= 0.1
        binary_imgs = post_process(binary)
            ...
            for x in range(width):
                for y in range(height):
                    if pixel_inside_FOV(i,x,y,FOVs):
                        ...
                        new_binary_imgs.append(binary_imgs[i,:,y,x])
        new_pred_imgs = np.asarray(new_pred_imgs)
        new_pred_masks = np.asarray(new_pred_masks)
        new_binary_imgs = np.asarray(new_binary_imgs)
        return new_pred_imgs, new_pred_masks, new_binary_imgs
    else:
        new_pred_imgs = []
        new_pred_masks = []
        for i in range(data_imgs.shape[0]):  # loop over the all test images
        ...
        new_pred_imgs = np.asarray(new_pred_imgs)
        new_pred_masks = np.asarray(new_pred_masks)
        return new_pred_imgs, new_pred_masks, None
