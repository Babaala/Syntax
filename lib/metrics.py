...
class Evaluate():
    def __init__(self,save_path=None, lc=False):
        self.target = None
        self.output = None
    ...
    def add_batch(self,batch_tar,batch_out, binary_images=None):
        ...
        if self.lc and binary_images is not None:
    ...
    def confusion_matrix(self):
        #Confusion matrix
        # [0,0]是TN，[0,1]是FP，[1,0]是FN，[1,1]是TP
        # N, w, h , _ = self.binary_images.shape
        if self.lc and self.binary_images is not None:  # tzl 添加额binary_images is not None 的判定
            y_pred = self.binary_images
        else:
        ...
        mIoU = 0
        if float(confusion[1, 1] + confusion[1, 0] + confusion[0, 1]) != 0:
            mIoU = float(confusion[1, 1]) / float(confusion[1, 1] + confusion[1, 0] + confusion[0, 1])
        # print("Precision: " +str(precision))
    ...
    def f1_score(self):
        if self.lc and self.binary_images is not None:
            pred = self.binary_images
        else:
            pred = self.output>=self.threshold_confusion
